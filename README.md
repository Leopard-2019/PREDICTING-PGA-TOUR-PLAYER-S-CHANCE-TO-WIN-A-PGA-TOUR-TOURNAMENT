...
<p>2. It is thougth that the best classification model is a close call between the K-Nearest Neighbor, and Random Forest which ranks first in all metrics. However, the elapsed time spent by Random Forest is quite large compare to improvement in metrics achieved. Also, K-Nearest Neighbor ranked the most widely known important golf player's statistics among the top five to win a PGA golf tournament. AS a result, it is thought that the winner must be K-Nearest Neighbor. All models performed relative poorly in the recall score, which is expected when dealing with a imbalanced dependent target variable.</p>
